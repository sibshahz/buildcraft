...
async function seed() {
  ...
  const industryData = [
    ...
    { 
      ...
      slug: 'residential', 
      icon: 'villa', 
      description: 'Tailored luxury for private Emirati villas, combining comfort with elite architectural design.', 
      longDescription: generateLexicalContent('For private residences, BuildCraft offers exclusivity and comfort. Our residential collection includes hand-tufted rugs, silk-blend carpets, and premium wood flooring. We work closely with interior designers to match your personal style with the highest quality materials available on the global market.'),
      image: IMAGES.villa 
    },
    { 
      ...
      slug: 'mosques', 
      icon: 'mosque', 
      description: 'Durable and spiritually inspired carpeting for sacred community and prayer facilities.', 
      longDescription: generateLexicalContent('Spiritual spaces require careful consideration of both aesthetics and durability. Our mosque carpets are designed with high-density fibers to withstand heavy use while maintaining their softness and intricate patterns. We provide custom alignments and designs that respect traditional values while incorporating modern durability standards.'),
      image: IMAGES.mosque 
    },
    { 
    ...
    { 
      ...
      slug: 'institutional', 
      icon: 'graduation', 
      description: 'Safe, sustainable, and professional-grade flooring for regional public and educational institutions.', 
      longDescription: generateLexicalContent('Education and public sector projects prioritize safety, hygiene, and longevity. Our slip-resistant and easy-to-clean flooring solutions meet all UAE safety regulations. We provide specialized coatings and materials that resist bacterial growth, making them ideal for schools, clinics, and government offices.'),
      image: IMAGES.office 
    },
  ]
  ...
  const serviceData = [
    ...
    { 
      ...
      slug: 'hospitality', 
      category: 'Commercial', 
      description: 'World-class flooring solutions tailored for the global luxury hospitality sector.', 
      content: generateLexicalContent(
    ...
    data: {
      ...
      layout: [
        ...
        },
        {
          ...
          content: generateLexicalContent('BuildCraft Flooring & Décor stands as a testament to Emirati reliability and architectural excellence. We represent the pinnacle of flooring solutions across the seven emirates.'),
          image: heroMedia.id,
          stats: [
  ...
  const findIndustryId = (slug: string) => industryDocs.docs.find(d => d.slug === slug)?.id || industryDocs.docs[0].id
  ...
  const projectData = [
    {
      ...
      content: 'We were commissioned to design and install custom-woven Axminster carpets for the Grand Emirates Ballroom. The design reflects traditional Emirati patterns with a modern gold-accented twist, providing both acoustic excellence and a regal atmosphere.\n\nThe project required precision installation around circular structural columns and integrated stage flooring, completed within a tight 14-day window to accommodate scheduled international summits.',
    ...
    },
    {
      ...
      content: 'For this 5,000 sqm office space, we utilized carbon-neutral modular carpet tiles in varying grey tones to define different work zones and pathways. The client required a low-VOC, sustainable solution that allowed for frequent access to underfloor power and data cabling.\n\nThe result is a productive, quiet, and ergonomically superior office environment that has become a benchmark for workplace design in the region.',
    ...
    },
    {
      ...
      content: 'The Al Noor Mosque project involved carpeting the entire prayer hall with custom-dyed green carpets featuring gold Saf (alignment) lines. We used our signature high-twist yarn technology to ensure the carpet remains plush for years to come despite daily heavy footfall.\n\nAll installation was completed during off-peak hours to ensure that daily prayers remained uninterrupted, showcasing our commitment to community respect and service excellence.',
  ...
  for (const item of projectData) {
    ...
    const galleryMedias = await Promise.all(
      item.images.map((img, i) => uploadImage(payload, img, `${item.title} Gallery ${i}`, `${item.slug}-gal-${i}`))
    )
